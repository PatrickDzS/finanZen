...
// Weights for Zen Score calculation (re-introducing goals)
const SAVINGS_WEIGHT = 0.35;
const EXPENSE_WEIGHT = 0.25;
const GOALS_WEIGHT = 0.25;
const DIVERSIFICATION_WEIGHT = 0.15;

// Function to calculate the Zen Score and its components
const calculateZenScore = (income: number, totalExpenses: number, investments: Investment[], goals: Goal[]) => {
    ...
    // c. Goal Progress Score
    let totalGoalProgress = 0;
    if (goals.length > 0) {
        const totalTarget = goals.reduce((sum, g) => sum + g.target, 0);
        const totalCurrent = goals.reduce((sum, g) => sum + g.currentAmount, 0);
        totalGoalProgress = totalTarget > 0 ? totalCurrent / totalTarget : 0;
    }
    const goalsScore = totalGoalProgress * GOALS_WEIGHT * 100;

    // d. Investment Diversification Score
    ...
    const totalScore = Math.round(savingsScore + expenseScore + goalsScore + diversificationScore);
...
const ZenScorePage: React.FC<ZenScorePageProps> = ({ income, totalExpenses, investments, goals }) => {
    // Zen Score calculation
    const { totalScore, savingsScore, expenseScore, goalsScore, diversificationScore } = calculateZenScore(income, totalExpenses, investments, goals);
    ...
    const tooltipContent = (
        <div className="text-left p-2 space-y-2 max-w-sm">
            ...
            <p className="text-xs text-slate-300">Sua pontuação de saúde financeira é uma média ponderada de quatro pilares:</p>
            <ul className="space-y-2 text-xs mt-2">
                ...
                <li className="flex items-start"><Lucide.ShieldCheck size={14} className="text-blue-400 mr-2 mt-0.5 flex-shrink-0" /><p><strong>Controle de Despesas ({EXPENSE_WEIGHT * 100}%):</strong> <span className="font-mono">{expenseScore.toFixed(0)}/{EXPENSE_WEIGHT * 100} pts</span><br/><span className="text-slate-400">Pontua o quão bem suas despesas se encaixam na sua renda.</span></p></li>
                 <li className="flex items-start"><Lucide.Target size={14} className="text-amber-400 mr-2 mt-0.5 flex-shrink-0" /><p><strong>Progresso nas Metas ({GOALS_WEIGHT * 100}%):</strong> <span className="font-mono">{goalsScore.toFixed(0)}/{GOALS_WEIGHT * 100} pts</span><br/><span className="text-slate-400">Mede o quão perto você está de alcançar seus objetivos financeiros.</span></p></li>
